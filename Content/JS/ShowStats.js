...
function showYokaiDetails(Yokai_Data) {
	...
	if (yokai.isNotBoss == 1){
		...
		if (typeof attack == 'undefined'){
			attack = attackDatabase['0x00000000']
		...
		if (typeof technique == 'undefined'){
			technique = techniqueDatabase['0x00000000']
			console.error("Cannot read properties of undefined. Perhaps the technique isn't written in the dictionary of yokai?")
		}
		if (typeof skill == 'undefined'){
			skill = abilitiesDatabase['0x00000000']
			console.error("Cannot read properties of undefined. Perhaps the skill isn't written in the dictionary of yokai?")
		}
		if (typeof inspirit == 'undefined'){
			inspirit = inspiritDatabase['0x00000000']
			console.error("Cannot read properties of undefined. Perhaps the inspirit isn't written in the dictionary of yokai?")
		}
		if (typeof soultimate == 'undefined'){
			soultimate = soultimateDatabase['0x00000000']
			console.error("Cannot read properties of undefined. Perhaps the soultimate isn't written in the dictionary of yokai?")
		}
		...
		const details = `
			...
			<p style="display: flex; align-items: center;"><a href="./MoveiInfoPage.html?moveType=2&id=${inspirit.ID}" style="color:var(--side-buttons-color)">Inspirit: ${inspirit.Command}</a> | Inspirit Level: ${inspirit.Tier} | Description: ${(inspirit.Description).replaceAll('\\n','\n')} <img style="position: relative;"src="Content/Graphics/InspiritImages/${GenericInspiritIDs[inspirit.GenericEffectID]}.png"> | Inspirit Probability: ${yokai.BaseInspiritPercent}%</p>
