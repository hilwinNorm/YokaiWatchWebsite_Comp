...
function ShowSlotInfo(slot){
	...
	if ((!slotStats || Object.keys(slotStats).length === 0) || (EditingSlots == false)) {
		if (Object.keys(slotStats).length === 0){slotStats = {}}
        if (slotName && slotName !== "null") {slotStats = yokaiDatabase[slotParamID];}
		slotStats.Name = slotName;
		slotStats.level=60;
		slotStats.attitude=0;
		slotStats.HP_IV=8;
		slotStats.STR_IV=8;
		slotStats.SPR_IV=8;
		slotStats.DEF_IV=8;
		...
		slotStats.DEF_GYM=0;
		slotStats.SPD_GYM=0;
		slotStats.equipment=slotStats.ItemSlots;
	...
	console.log([BHP, BSTR, BSPR, BDEF, BSPD], slotStats)
	...
	<div style="display: grid;">
		<div style="display: grid; align-items: center;.">
			...
			<h3 class="SlotInfo-Text">${slotName}</h3>
			</div>
		...
		<div style="display:grid; justify-items: center;">
			<img width="55px" height="auto" src="Content/Graphics/Equip_Image.png">
	...
	<div class="SlotInfo-Text" style="display: flex; flex-direction: column; justify-content: space-evenly;">
	<h4 for="yokai-attitude">Attitude: <select id="yokai-attitude"></select></h4>
	...
	<div style="display: flex">
		<div class="yokai-attributes" style="display: grid; margin: 10px; padding: 0px 8px;">
			<h3 data-stat="${BHP/2}" class="SlotInfo-Text Stats">HP: ${Math.round(BHP) + Attitudes[slotStats.attitude].boost[0]}</h3>
			<h3 data-stat="${BSTR}" class="SlotInfo-Text Stats">STR: ${Math.round(BSTR) + Attitudes[slotStats.attitude].boost[1]}</h3>
			<h3 data-stat="${BSPR}" data-stat="${BHP}" class="SlotInfo-Text Stats">SPR: ${Math.round(BSPR) + Attitudes[slotStats.attitude].boost[2]}</h3>
			<h3 data-stat="${BDEF}" class="SlotInfo-Text Stats">DEF: ${Math.round(BDEF) + Attitudes[slotStats.attitude].boost[3]}</h3>
			<h3 data-stat="${BSPD}" class="SlotInfo-Text Stats">SPD: ${Math.round(BSPD) + Attitudes[slotStats.attitude].boost[4]}</h3>
		</div>
		<div class="yokai-attributes" style="justify-content: flex-start; flex-direction: column; margin: 10px; width: 120px;">
			<h3 class="SlotInfo-Text">Gym STR: <input style="width:30px" type="number" value=${slotStats.STR_GYM} placeholder="STR" id="yokai-STR_Gym" class="GYM"></h3>
		...
		<div class="yokai-attributes" style="justify-content: flex-start; flex-direction: column; margin: 10px; width: 120px;">
			<h3 class="SlotInfo-Text">IV HP: <input style="width:30px;" type="number" value=${slotStats.HP_IV} placeholder="HP" id="IV-HP" class="IV"></h3>
	...
	slotInfo_div.querySelectorAll('.Stats').forEach(function(stat){
...
function showPage(yokais) {
	...
	for (const [key,yokai] of Object.entries(yokais)) {
		...
		divMain.querySelectorAll('.Stats').forEach(function(stat){
		var dc = Math.round(parseInt(stat.getAttribute("data-stat")))
