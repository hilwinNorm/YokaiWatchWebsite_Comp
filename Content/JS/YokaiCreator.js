...
function SetOptions_Inspirit(GivenAttack=null){
	if (GivenAttack!=null){
		...
		document.getElementById('info-inspirit').innerHTML = `
        <div><span class="info-label">Tier:</span> ${GivenAttack.Tier}</div>
        <div><span class="info-label">Target:</span> ${GivenAttack.Target}</div>
        <div><span class="info-label">Desc:</span> ${GivenAttack.Description}</div>
        `;
		return;
	}
