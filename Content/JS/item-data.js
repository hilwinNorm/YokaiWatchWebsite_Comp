...
function showItemDetails(database, item) {
	...
	ItemIcon.src =`${ImgPath}item_${(item.IconPosX + (item.IconPosY * 16) + 1).toString().padStart(3,'0')}.xi.00.png`;
...
window.addEventListener('load', function (event) {
	switch (parseInt(databaseType)){
		...
		case 1:
			...
			break;
		case 2:
			var ItemData = soulgemDatabase[urlParams.get('item')]
			showItemDetails(soulgemDatabase, ItemData)
			break;
	}
